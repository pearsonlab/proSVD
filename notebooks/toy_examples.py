...
V_true, _ = np.linalg.qr(np.random.normal(size=(t_latent[0],t_latent[0])))
...
for row, curr_t in enumerate(t_latent):
    ...
    # run it
    # projs, frac_vars, derivs, proj_mats = pro.run(xs, l1)   
    ...
    # comparing projection matrices over time - just norm(Q_prev@Q_prev.T - Q@Q.T))
    ...
    for j in range(2):
        ...
        for p in range(regime_changes+1): 
            ...
            text_x_loc = (regime_start + (curr_t)-125) / l # in data coords for ease
    ...
    for j, currax in enumerate(ax[row, :]):
        for lineloc in [(curr_t*(i+1)-l1)/l for i in range(regime_changes+1)]:
            if j == 0:
                lineloc *= l
